...
class MediaInstance {
    ...
    start(options) {
        this.actor.context.internal.lookupAsset(this.mediaAssetId).created.then(() => {
            ...
            log_1.log.error('app', `Start failed ${this.actor.id}. ${(reason || '').toString()}`.trim());
        });
        return this;
    }
    /**
    ...
    setState(options) {
        this.actor.context.internal.lookupAsset(this.mediaAssetId).created.then(() => {
            ...
            log_1.log.error('app', `SetState failed ${this.actor.id}. ${(reason || '').toString()}`.trim());
        });
    }
    /**
    ...
    stop() {
        this.actor.context.internal.lookupAsset(this.mediaAssetId).created.then(() => {
            ...
            log_1.log.error('app', `Stop failed ${this.actor.id}. ${(reason || '').toString()}`.trim());
        });
    }
}
